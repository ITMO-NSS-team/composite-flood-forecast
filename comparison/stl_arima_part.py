...
for station in stations:
    ...
    stlf = STLForecast(sub_df['stage_max'], ARIMA, period=365, model_kwargs={"order": (4, 1, 4)})
    res = stlf.fit()
    forecasts = np.array(res.forecast(forecast_len))

    #stlf2 = STLForecast(sub_df['stage_max'], AutoReg, period=345, model_kwargs={"lags": 8})
    #res2 = stlf2.fit()
    #forecasts2 = res2.forecast(forecast_len)
    ...
    plt.plot(real_dates, forecasts, c='r', linewidth=1, label='ARIMA forecast')
    #plt.plot(real_dates, forecasts2, c='orange', linewidth=1, label='AR forecast')
